...
- **消費電力計**（Plus 限定）: SoC の消費電力を燃料計スタイルで表示。ピークを 100% と
  する相対表示で、ピーク記憶方式をメニューから選べます（5分 / 15分 / 30分 / 1時間の
  スライディングウィンドウ、または期間を区切らず観測最大を永続保持する「記憶」。既定は「記憶」）。
  ピーク更新中は警告灯が点滅します
...
- **Power meter** (Plus only): shows SoC power draw as a car-style fuel gauge,
  relative to the peak (100%). The peak-hold method is selectable from the menu
  (a 5 / 15 / 30 min or 1 hour sliding window, or "Keep" which retains the
  observed maximum indefinitely; default is "Keep"). The warning lamp blinks
  while a new peak is being set
