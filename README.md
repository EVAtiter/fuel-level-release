...
バッテリー温度計は非公開 API（バッテリーの内部温度取得）を使用するため、
App Store 版には搭載できません。Plus 版は Developer ID 署名 + Apple 公証済みで配布しています。
...
The battery thermometer relies on an undocumented API (reading the
battery's internal temperature) and cannot be included in the App Store
build. The Plus edition is Developer ID signed and notarized by Apple.
